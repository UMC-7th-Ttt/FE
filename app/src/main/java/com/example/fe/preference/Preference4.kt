...
class Preference4 : AppCompatActivity() {
    ...
    private val images = listOf(
        ...
        R.drawable.ic_bookcover_example4, // 네 번째 이미지
        R.drawable.ic_bookcover_example5, // 다섯 번째 이미지
        R.drawable.ic_bookcover_example6  // 여섯 번째 이미지
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 페이지가 선택될 때마다 설명 텍스트 업데이트
        viewPager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                descriptionText.text = descriptions[position]  // 해당 페이지에 맞는 설명을 설정
            }
        })
